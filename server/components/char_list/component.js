class CharList extends HTMLElement {
	...
	switchTheme(themeId, studioId) {
		this.themeSelBtn.innerHTML = "";
			...
			image.src = `/pages/img/themes/icons/${studioId}.webp`;
			this.themeSelBtn.appendChild(image);
		$.get(`/api/char/list?themeId=${themeId}`)
			.done((data) => {
				this.chars = data;
				this.killChildren();
				this.switchPage(0);
			...
			.fail((data) => {
				console.error(data);
				this.displayError("An error has occured trying to load the character list.");
			});
	}
