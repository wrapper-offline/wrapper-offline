...
export default class AppSetting extends LitElement {
			...
			${this.options == "binary" ? 
				...
				html`<select class="setting" @change=${this._update}>
					${Object.entries(this.options).map(([val, name]) =>
						html`<option value=${val} ?selected=${this.value && (this.value.toString() == val)}>${name}</option>`
					)}
				</select>`}
		`;
	}
	...
	firstUpdated() {
		...
		if (!this.local) {
			SettingsController.connect(this);
		} else {
	...
	_update() {
		...
		if (this.local) {
			...
			if (this.id == "DARK_MODE") {
				...
				appBody.refreshDarkMode();
				document.body.classList.add("dark_transition");
				setTimeout(() => {
					document.body.classList.remove("dark_transition");
				}, 200);
			}
		} else {
			SettingsController.set(this.id, this.value.toString());
		}
	}
};
