import { css, html, LitElement, render } from "../../media/js/lit-core.min.js";
...
class TabbedNavigation extends LitElement {
	...
	render() {
		...
		this.navButtons.classList.add("nav_buttons");
		return html`
			${this.navButtons}
			<slot></slot>
		`;
	...
	firstUpdated() {
		for (const child of this._slottedChildren) {
			const name = child.attributes.getNamedItem("name")?.value;
			if (
				child.tagName != "NAV" ||
			...
			button.addEventListener("click", () => this._switchTab(name));
